...
namespace WebApplication4.Controllers
{
    public class HomeController : Controller
    {
        ...
        // 🔹 ВСПОМОГАТЕЛЬНЫЙ МЕТОД: возвращает текст уведомления, если пользователь заблокирован
        ...
        public async Task<IActionResult> Index(string search, int? appTypeId, int? complexityId, int? statusId, string sortBy)
        {
            ...
            var projects = _context.Projects
                .Where(p => p.ModerationStatus == ModerationStatus.Approved && !p.ApplicationUser.IsBanned)
            ...
            {
                projects = projects.Where(p => !blockedUserIds.Contains(p.ApplicationUserId!));
            }
        ...
        // 🔹 СТРАНИЦА ДОСТУПНА ВСЕМ, ВКЛЮЧАЯ ЗАБЛОКИРОВАННЫХ
        ...
        // 🔹 ЛЕНТА ТОЛЬКО ДЛЯ НЕЗАБЛОКИРОВАННЫХ
        [Authorize]
        public async Task<IActionResult> Feed()
        {
            var user = await _userManager.GetUserAsync(User);
            if (user?.IsBanned == true) return RedirectToAction("Index", new { banned = 1 });
            ...
            {
                TempData["Info"] = "Вы пока ни на кого не подписаны. Найдите интересных разработчиков!";
